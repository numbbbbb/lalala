...
LOGGING_LEVEL_CONFIG = objectify({
    ...
    'job_worker': 'info', # just control how pyres do the logging, our code still controlled by LOGGING_LEVEL_CONFIG.lalala
    'lalala_postgresql': {
...
def env_config(person_website_start_port, person_website_process_count, person_website_domain, person_website_domain_port, persist_store_redis_host,
        persist_store_redis_port, memory_cache_redis_host, memory_cache_redis_port, lalala_postgresql_version, lalala_postgresql_host,
        lalala_postgresql_port, queue_type, queue_host, queue_port, resweb_domain, resweb_domain_port, resweb_host, resweb_port):
...
def lalala_periodic_job_scheduler_program(config):
    return periodic_job_scheduler_program(LOGGING_LEVEL_CONFIG.lalala, application_config=lalala_config(config))

def lalala_job_worker_program(worker_name, queue_names, config, run_as=None, count=1, timeout=180):
    return job_worker_program(worker_name, LOGGING_LEVEL_CONFIG.job_worker, LOGGING_LEVEL_CONFIG.lalala, config.queue_host, config.queue_port,
        queue_names=queue_names, application_config=lalala_config(config), run_as=run_as, count=count, timeout=timeout)


def routines_job_worker_program(config, count=1):
    return lalala_job_worker_program('routines',
        [
...
def transactional_email_worker_program(config, count=1):
    return lalala_job_worker_program('transactional_email', ['send_transactional_email'], config, count=count)
...
def lalala_postgresql_program(config, more_config=None):
    more_config = more_config or {}
    return log_rotated_postgresql_program('lalala', config.lalala_postgresql_version, config.lalala_postgresql_host, config.lalala_postgresql_port,
        ...
        log_min_duration_statement=LOGGING_LEVEL_CONFIG.lalala_postgresql.log_min_duration_statement, **more_config)
...
def person_website_programs(config):
    return website_programs('person', LOGGING_LEVEL_CONFIG.lalala, application_config=lalala_config(config), start_port=config.person_website_start_port,
...
def lalala_config(config):
    lalala_config_ = objectify({
        'queue_client': {
    ...
    for purpose in REDIS_CLIENTS:
        lalala_config_['{}_redis_client'.format(purpose)] = {
    ...
    for purpose in PERSON_WEBSITE_BUCKETS:
        lalala_config_['{}_bucket'.format(purpose)] = {
            'type': 'filesystem',
    ...
    for purpose in POSTGRESQL_CLIENTS:
        lalala_config_['{}_database_client'.format(purpose)] = {
    ...
    for purpose in WEBSITES:
        lalala_config_['{}_website'.format(purpose)] = {
            ...
            'master_template_directory': VEIL_HOME / 'src' / 'lalala' / 'website' / purpose,
            'prevents_xsrf': True,
            'recalculates_static_file_hash': True,
            'clears_template_cache': True
        }
    return lalala_config_
